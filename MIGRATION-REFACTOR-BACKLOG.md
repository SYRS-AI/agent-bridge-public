...
### 6a. legacy wrapper script가 아직 gateway 시대 delivery semantics를 직접 품고 있다
- 현상: `runtime/scripts/call-shopify.sh`, `cron-failure-monitor.sh`, `patch-a2a-bridge.sh`, `shopify-a2a-bridge.sh` 같은 스크립트는 여전히 `openclaw message send`, `sessions_send`, 별도 상태 파일을 전제로 한다.
- 비효율:
  - bridge queue / live Claude session / notifier 규칙과 wrapper 내부 규칙이 이중으로 존재한다.
  - `.openclaw` 경로를 다 옮겨도 wrapper contract 자체가 오래된 mental model을 계속 끌고 간다.
- 리팩터링 방향:
  - bridge-native notifier/queue contract를 기준으로 wrapper를 재작성하거나 폐기
  - 사람-facing 보고, A2A handoff, cron follow-up을 공통 helper로 수렴

### 6b. webhook/mail cluster가 아직 "지연 cron wrapper" mental model에 묶여 있다
- 현상: Gmail/Shopify/TracX webhook helper는 queue handoff와 delayed cron wrapper가 섞여 있다.
- 비효율:
  - webhook -> queue -> follow-up 의 단일 모델이 아니라 one-shot cron과 direct handoff가 혼재한다.
  - mailbot처럼 실시간성이 중요한 경로에서 "watch는 살아 있는데 triage task는 안 생김" 같은 장애가 난다.
- 리팩터링 방향:
  - webhook 진입점은 durable queue handoff를 기본으로 두고, 정말 필요한 지연 실행만 bridge-native one-shot schedule helper로 분리
  - `webhook_utils.py`가 bridge-native helper를 source of truth로 소유하게 만들기
