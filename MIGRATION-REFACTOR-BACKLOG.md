...
### 1. cron import 이후 legacy source 경로를 완전히 잊도록 남은 호환층을 정리해야 한다
- 현상: canonical cron store는 `~/.agent-bridge/cron/jobs.json`로 옮기고 있지만, import/rewrite 도구와 일부 문서에는 아직 legacy source 개념이 남아 있다.
- 비효율:
