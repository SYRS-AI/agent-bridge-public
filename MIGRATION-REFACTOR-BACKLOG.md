...
### 1. cron source of truth가 아직 `~/.openclaw/cron/jobs.json`
- 현상: Agent Bridge가 recurring cron을 실질적으로 관리하지만 저장소는 아직 legacy jobs file에 남아 있다.
- 비효율:
  - bridge-native cron store와 이중 구조다.
  - `rewrite-cron` 같은 migration pass가 계속 필요하다.
  - repo 입장에서 fresh install mental model이 흐려진다.
- 리팩터링 방향:
  - recurring cron의 canonical store를 `~/.agent-bridge/cron/jobs.json`로 단일화
  - import/migrate command는 별도 one-shot으로 분리
  - inventory/scheduler/CRUD 전부 bridge-native store 기준으로 수렴
