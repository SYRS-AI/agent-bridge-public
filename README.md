...
Claude Code 채널 플러그인은 설치만으로는 준비되지 않습니다. Discord/Telegram 봇 토큰과 접근 설정 파일이 있어야 MCP 서버가 연결됩니다. Agent Bridge는 `setup`과 에이전트 시작 시점에 필요한 Claude Code 플러그인을 자동으로 설치/활성화합니다.
