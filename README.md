...
> 참고: 사람이 tmux에 붙어 있는 상태에서 에이전트가 `exit`로 정상 종료되면 자동 재시작하지 않고 터미널로 돌아옵니다. 다시 열려면 `agb admin` 또는 `agb agent start <agent>`를 실행하세요. 강제로 내리려면 바깥 쉘에서 `agb kill <agent>`를 사용합니다.
