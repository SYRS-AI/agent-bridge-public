...
def render_agent_bridge_block(agent_dir: Path, session_type: str | None = None) -> str:
    ...
    lines = [
        ...
        *(
        ...
        "   - review/consult 답변(LGTM, findings, design analysis)도 예외가 아니다. `done --note`에만 적으면 요청자 inbox에 신호가 안 들어간다. 새 task로 전달하고 원 task는 `done --note \"결과를 task #<new>로 전달\"`로 닫는다. issue #179 참조.",
        "   - 요청자가 registered agent가 아니면 `task create --to`가 실패한다. fallback: active coordinator/admin에게 relay하거나 원 task를 `update --status blocked`로 표면화한다. done-note에 본문 묻기는 금지.",
        "4. **done**: `agb done <task_id> --note \"요약\"` — 반드시 note에 무엇을 했는지 기록",
        "- **조용한 done 금지**: 결과를 아무에게도 전달하지 않은 채 done만 치는 것은 금지",
        "- **done-note 결과 전달 금지**: 다른 에이전트 요청자면 본문은 새 task에 두고 note는 `<new task id>로 전달함` 포인터만 담는다.",
