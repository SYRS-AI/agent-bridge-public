...
이제 크론, 스킬, 스크립트, DB helper, secret 경로는 모두 "우리 런타임"으로 본다. 다만 현재 실제 소스가 legacy tree에 남아 있으므로, import source로만 표현한다.
...
### 1. 크론은 정의만 있고, 현재 live daemon이 지속적으로 스케줄을 먹고 있지 않다

- live daemon은 `BRIDGE_OPENCLAW_CRON_SYNC_ENABLED`가 꺼져 있으면 recurring cron sync를 돌리지 않는다.
- 현재 live `scheduler-state.json` 마지막 sync 시각은 `2026-04-05T18:27:56+09:00`에 멈춰 있다.
- bridge-native cron store는 비어 있다.
- 따라서 지금 대부분의 에이전트가 sleep인 것은 "할 일이 없어서"가 아니라 "recurring cron enqueue가 계속 생성되지 않아서"인 면이 크다.

### 2. 지금 cron sync를 바로 켜도 많은 잡이 실패할 가능성이 높다

현재 `~/.openclaw/cron/jobs.json` inventory 기준:
...
즉 "크론이 안 돈다"의 1차 원인은 sync가 꺼져 있기 때문이고, 2차 원인은 payload 자체가 아직 bridge-local runtime으로 옮겨지지 않았기 때문이다.
